...
auditRouter.get('/', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        ...
        const userIds = [...new Set(logs.map((log) => log.userId).filter((id) => Boolean(id)))];
...
auditRouter.delete('/retention', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        const superAdminEmail = process.env.SUPER_ADMIN_EMAIL;
        const isSuperAdmin = superAdminEmail && authUser.email === superAdminEmail;
        ...
        const days = Math.max(30, parseInt(req.query.days) || 90);
