...
function isRegistrationBoundToPayload(registration, payload) {
    return (registration.id === payload.registrationId &&
        registration.userId === payload.userId &&
        registration.eventId === payload.eventId);
}
function requireUuid(res, value, label) {
...
function resolveClientScannedAt(scannedAtLocal) {
    const now = new Date();
    ...
    if (parsedMs > nowMs + CLIENT_SCAN_FUTURE_TOLERANCE_MS ||
        parsedMs < nowMs - CLIENT_SCAN_MAX_AGE_MS) {
        return now;
    }
    return parsed;
}
function normalizeEventDays(eventDays) {
    if (!Number.isInteger(eventDays) || !eventDays || eventDays < 1)
        return 1;
    return Math.min(eventDays, 10);
}
function parseDayLabels(value, eventDays) {
...
function parseRequestedDayNumber(dayNumber) {
...
function resolveEffectiveDayNumber(dayNumber, eventDays, defaultToOne = true) {
    const parsed = parseRequestedDayNumber(dayNumber);
    if (parsed === null)
        return defaultToOne ? 1 : null;
    if (!Number.isInteger(parsed) || parsed < 1 || parsed > eventDays)
        return Number.NaN;
    return parsed;
}
async function syncRegistrationAttendance(registrationId) {
...
async function resolveStoredAttendanceTokenPayloads(tokens) {
...
async function resolveAttendancePayloadFromToken(token) {
...
// ────────────────────────────────────────────────────────────
router.get('/my-qr/:eventId', authMiddleware, requireFeature('attendance'), async (req, res) => {
    try {
...
// ────────────────────────────────────────────────────────────
router.post('/scan', authMiddleware, requireRole('CORE_MEMBER'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
        }
        const { token, bypassWindow, dayNumber } = req.body;
...
// ────────────────────────────────────────────────────────────
router.post('/scan-batch', authMiddleware, requireRole('CORE_MEMBER'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
        }
        const { scans, eventId, bypassWindow } = req.body;
        ...
        if (fallbackCandidates.length > 0) {
            const fallbackPayloadMap = await resolveStoredAttendanceTokenPayloads(fallbackCandidates.map((scan) => scan.token));
...
router.post('/scan-beacon', beaconLimiter, requireFeature('attendance'), express.text({ type: '*/*' }), async (req, res) => {
    try {
        ...
        let skippedCount = 0;
        let failedCount = 0;
        const fallbackPayloadMap = await resolveStoredAttendanceTokenPayloads(scans
            .filter((scan) => Boolean(scan && typeof scan === 'object' && typeof scan.token === 'string'))
            .map((scan) => scan.token));
        try {
            ...
            for (const scan of scans) {
                try {
                    ...
                    const scannedAt = resolveClientScannedAt(typeof scan.scannedAtLocal === 'string' ? scan.scannedAtLocal : undefined);
                    const eventDays = normalizeEventDays(registration.event.eventDays);
...
// ────────────────────────────────────────────────────────────
router.post('/manual-checkin', authMiddleware, requireRole('CORE_MEMBER'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
        }
        const { registrationId, dayNumber } = req.body;
...
// ────────────────────────────────────────────────────────────
router.patch('/unmark', authMiddleware, requireRole('CORE_MEMBER'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
...
// ────────────────────────────────────────────────────────────
router.patch('/bulk-update', authMiddleware, requireRole('CORE_MEMBER'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
        }
        const { registrationIds, action, dayNumber } = req.body;
        ...
        if (invalidDayTarget) {
            ...
            return ApiResponse.badRequest(res, `dayNumber must be between 1 and ${maxEventDays} for ${invalidDayTarget.registration.user.name}`);
        }
...
// ────────────────────────────────────────────────────────────
router.patch('/edit/:registrationId', authMiddleware, requireRole('CORE_MEMBER'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
        ...
        const { scannedAt, manualOverride, dayNumber } = req.body;
        const eventDays = normalizeEventDays(registration.event.eventDays);
        const effectiveDayNumber = resolveEffectiveDayNumber(dayNumber, eventDays, true);
        if (!effectiveDayNumber || Number.isNaN(effectiveDayNumber)) {
            return ApiResponse.badRequest(res, `dayNumber must be between 1 and ${eventDays}`);
        ...
        if (!existingDay) {
            await prisma.dayAttendance.create({
                data: {
                    ...
                    scannedAt: updateData.scannedAt ?? null,
            ...
            await prisma.dayAttendance.update({
                ...
                data: {
                    ...(updateData.scannedAt !== undefined && { scannedAt: updateData.scannedAt }),
                    ...(updateData.manualOverride !== undefined && { manualOverride: updateData.manualOverride }),
...
// ────────────────────────────────────────────────────────────
router.post('/regenerate-token/:registrationId', authMiddleware, requireRole('ADMIN'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
...
// ────────────────────────────────────────────────────────────
router.post('/regenerate-tokens/event/:eventId', authMiddleware, requireRole('ADMIN'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
        ...
        for (let index = 0; index < registrations.length; index += ATTENDANCE_REGENERATE_BATCH_SIZE) {
            ...
            await prisma.$transaction(batch.map((registration) => prisma.eventRegistration.update({
                where: { id: registration.id },
                data: {
                    attendanceToken: generateAttendanceToken(registration.userId, registration.eventId, registration.id),
                },
            })));
...
// ────────────────────────────────────────────────────────────
router.get('/search', authMiddleware, requireRole('CORE_MEMBER'), async (req, res) => {
    try {
        const { q, eventId, page: pageParam } = req.query;
...
// ────────────────────────────────────────────────────────────
router.get('/live/:eventId', authMiddleware, requireRole('CORE_MEMBER'), async (req, res) => {
    try {
        const { eventId } = req.params;
        if (!requireUuid(res, eventId, 'event ID')) {
            return;
...
// ────────────────────────────────────────────────────────────
router.get('/event/:eventId/full', authMiddleware, requireRole('CORE_MEMBER'), async (req, res) => {
    try {
        const { eventId } = req.params;
        if (!requireUuid(res, eventId, 'event ID')) {
            return;
        ...
        if (totalRegistrations > ATTENDANCE_FULL_LIST_LIMIT) {
            return ApiResponse.badRequest(res, `Full attendance list is limited to ${ATTENDANCE_FULL_LIST_LIMIT} registrations. Use search or export for larger events.`);
        }
        const registrations = await prisma.eventRegistration.findMany({
            where: { eventId },
            include: {
...
// ────────────────────────────────────────────────────────────
router.get('/event/:eventId/export', authMiddleware, requireRole('CORE_MEMBER'), async (req, res) => {
    try {
        const { eventId } = req.params;
        if (!requireUuid(res, eventId, 'event ID')) {
            return;
        ...
        if (totalRegistrations > ATTENDANCE_EXPORT_LIMIT) {
            return ApiResponse.badRequest(res, `Attendance export is limited to ${ATTENDANCE_EXPORT_LIMIT} registrations per request.`);
        }
        const registrations = await prisma.eventRegistration.findMany({
            where: { eventId },
            include: {
        ...
        if (includeDayColumns) {
            for (const dayNumber of targetDays) {
                ...
                columns.push({ header: `${dayLabel} Present`, key: `${dayKeyPrefix}Attended`, width: 16 }, { header: `${dayLabel} Scanned At`, key: `${dayKeyPrefix}ScannedAt`, width: 24 }, { header: `${dayLabel} Manual`, key: `${dayKeyPrefix}Manual`, width: 16 });
            }
        ...
        buildWorksheet('Participants', registrations.filter((registration) => registration.registrationType === RegistrationType.PARTICIPANT));
        buildWorksheet('Guests', registrations.filter((registration) => registration.registrationType === RegistrationType.GUEST));
...
// ────────────────────────────────────────────────────────────
router.post('/email-absentees/:eventId', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
        ...
        const { eventId } = req.params;
        const { subject, body, dayNumber } = req.body;
        if (!requireUuid(res, eventId, 'event ID')) {
            return;
        }
        ...
        for (let i = 0; i < absentees.length; i += BATCH_SIZE) {
            ...
            const batchResults = await Promise.allSettled(batch.map(async (reg) => {
...
// ────────────────────────────────────────────────────────────
router.get('/event/:eventId/certificate-recipients', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const { eventId } = req.params;
        if (!requireUuid(res, eventId, 'event ID')) {
            return;
...
router.get('/event/:eventId/summary', async (req, res) => {
    try {
        ...
        const [total, attended, daySummary] = await Promise.all([
            ...
            Promise.all(Array.from({ length: eventDays }, (_, index) => index + 1).map(async (dayNumber) => ({
                dayNumber,
...
// ────────────────────────────────────────────────────────────
router.post('/backfill-tokens', authMiddleware, requireRole('ADMIN'), requireFeature('attendance'), async (req, res) => {
    try {
        const admin = getAuthUser(req);
        if (!admin) {
            return ApiResponse.unauthorized(res);
