...
const updateAnnouncementSchema = createAnnouncementSchema.partial().refine((data) => Object.keys(data).length > 0, { message: 'At least one field is required' });
...
announcementsRouter.get('/:id', async (req, res) => {
    try {
        ...
        const includeOptions = { creator: { select: { id: true, name: true, avatar: true } } };
...
// Create announcement
announcementsRouter.post('/', authMiddleware, requireRole('CORE_MEMBER'), async (req, res) => {
    try {
        ...
        const existingSlugs = (await prisma.announcement.findMany({
            ...
            select: { slug: true },
        })).map((announcement) => announcement.slug).filter(Boolean);
        ...
        if (announcement.slug)
            submitUrl(`/announcements/${announcement.slug}`);
...
async function sendAnnouncementEmailsAsync(announcement) {
    try {
        ...
        await emailService.sendAnnouncementToAll(emails, announcement.title, announcement.body, announcement.priority, announcement.slug || '', announcement.shortDescription || undefined, announcement.imageUrl || undefined, announcement.tags || []);
...
// Update announcement
announcementsRouter.put('/:id', authMiddleware, requireRole('CORE_MEMBER'), async (req, res) => {
    try {
        const authUser = getAuthUser(req);
...
// Delete announcement
announcementsRouter.delete('/:id', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const authUser = getAuthUser(req);
