...
const normalizeExplicitOverride = (value) => {
    return value?.trim() ?? '';
};
...
const toCleanSlugBase = (raw) => generateSlug(raw) || 'team-member';
const resolveUniqueTeamSlug = async (raw, excludeId) => {
    const baseSlug = toCleanSlugBase(raw);
    const existingSlugs = (await prisma.teamMember.findMany({
        where: {
...
const sanitizeRichContent = (data) => {
    const result = { ...data };
    ...
    if (typeof result.website === 'string') {
        result.website = sanitizeUrl(result.website) || null;
    }
    return result;
};
...
const mergeWithUserData = (teamMember, includeSyncMetadata = true) => {
    if (!teamMember.user)
        return teamMember;
...
teamRouter.get('/me', authMiddleware, async (req, res) => {
    try {
...
teamRouter.get('/meta/teams', async (_req, res) => {
    try {
...
// Create team member
teamRouter.post('/', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        ...
        if (teamMember.slug)
            submitUrl(`/team/${teamMember.slug}`);
...
teamRouter.put('/:id', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        ...
        const shouldRegenerateFromName = normalizedName !== undefined &&
            normalizedName.length > 0 &&
            normalizedName !== existing.name;
        const slugSource = explicitSlug.length > 0
            ? explicitSlug
        ...
        const teamMember = await prisma.teamMember.update({
            ...
            data: {
                ...
                ...(bio !== undefined && { bio: sanitizedData.bio }),
                ...(vision !== undefined && { vision: sanitizedData.vision }),
                ...(story !== undefined && { story: sanitizedData.story }),
                ...(expertise !== undefined && { expertise: sanitizedData.expertise }),
                ...(achievements !== undefined && { achievements: sanitizedData.achievements }),
                ...(website !== undefined && { website: sanitizedData.website }),
            },
        });
...
teamRouter.put('/:id/profile', authMiddleware, async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        const { id } = req.params;
...
teamRouter.patch('/:id/link-user', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        const { id } = req.params;
        ...
        const updateData = { userId: userId || null };
        if (userId) {
...
teamRouter.patch('/reorder', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const authUser = getAuthUser(req);
        ...
        await prisma.$transaction(members.map(({ id, order }) => prisma.teamMember.update({ where: { id }, data: { order } })));
...
// Delete team member
teamRouter.delete('/:id', authMiddleware, requireRole('ADMIN'), async (req, res) => {
    try {
        const authUser = getAuthUser(req);
