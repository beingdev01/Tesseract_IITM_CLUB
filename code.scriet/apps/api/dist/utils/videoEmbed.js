...
function setAllowedParams(target, source, keys) {
...
function normalizeYouTubeEmbedUrl(source, host) {
...
function normalizeVimeoEmbedUrl(source, host) {
    const segments = source.pathname.split('/').filter(Boolean);
...
function normalizeLoomEmbedUrl(source) {
    const segments = source.pathname.split('/').filter(Boolean);
    const videoId = segments[0] === 'embed'
        ? segments[1] ?? null
        : segments[0] === 'share'
            ? segments[1] ?? null
            : null;
...
export function normalizeTrustedVideoEmbedUrl(rawUrl) {
