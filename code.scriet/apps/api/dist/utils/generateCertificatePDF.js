...
import { Document, Page, View, Text, Image, Svg, Polygon, Line, Font, renderToBuffer, } from '@react-pdf/renderer';
...
export async function initFonts() {
    ...
    if (!fontInitPromise) {
        fontInitPromise = (async () => {
            const failures = [];
            const attemptRegistration = (name, register) => {
                try {
                    register();
                }
                catch (err) {
...
export function resetFontInitializationForTests() {
    fontsInitialized = false;
    fontInitPromise = null;
}
...
function getDescriptionLayoutConfig(description) {
    if (!description) {
        return {
            fontSize: 19,
            lineHeight: 1.4,
...
export function formatPosition(pos) {
    ...
    if (p === '1' || p === '1st' || p === '1st place' || p === 'first' || p === 'first place')
        return 'First Place';
    if (p === '2' || p === '2nd' || p === '2nd place' || p === 'second' || p === 'second place')
        return 'Second Place';
    if (p === '3' || p === '3rd' || p === '3rd place' || p === 'third' || p === 'third place')
        return 'Third Place';
    if (p === '4' || p === '4th' || p === 'fourth')
        return 'Fourth Place';
    if (p === '5' || p === '5th' || p === 'fifth')
        return 'Fifth Place';
...
const POSITION_TEMPLATE_HIGHLIGHT_STYLE = {
    fontWeight: 700,
    color: C.maroon,
    fontStyle: 'normal',
};
function hasInlineTokens(token) {
    return Array.isArray(token.tokens);
}
function hasInlineText(token) {
    return typeof token.text === 'string';
}
function getTokenTextOrRaw(token) {
...
function normalizeHtmlDescription(raw) {
    return raw
        .replace(/<br\s*\/?\s*>/gi, '\n')
...
function getNestedTokens(token) {
    return hasInlineTokens(token) ? token.tokens : [];
}
function getListItems(token) {
...
function getListStart(token) {
    const start = token.start;
    return typeof start === 'number' ? start : 1;
}
function isOrderedList(token) {
    return token.ordered === true;
}
function pushStyledText(nodes, text, key, style) {
    if (!text) {
        return;
    }
...
function renderTextWithTemplateMarkers(text, keyPrefix, style) {
    const nodes = [];
    let lastIndex = 0;
    let markerIndex = 0;
...
function renderMarkdownInlineTokens(tokens, keyPrefix, style = {}) {
    const nodes = [];
...
function parseMarkdownDescription(description) {
    try {
...
// ── DESCRIPTION BUILDER ────────────────────────────────────────────────────────
export function buildDescription(data, type, hasEventName = true) {
...
export async function generateCertificatePDF(data) {
    ...
    return renderToBuffer(React.createElement(Document, null, React.createElement(Page, {
        size: [W, H],
    ...
    }, 'CERTIFICATE'), React.createElement(Text, {
        style: {
    ...
    React.createElement(Svg, { width: 300, height: 12 }, React.createElement(Line, {
    ...
    }, 'THIS CERTIFICATE IS PROUDLY PRESENTED TO'), ...(recipientName
        ? [
            React.createElement(Text, {
                key: 'recipient-name',
                style: {
    ...
    }, `Verify at ${verifyDomain}`)), React.createElement(View, { style: { width: sideReserveWidth } }))), 
