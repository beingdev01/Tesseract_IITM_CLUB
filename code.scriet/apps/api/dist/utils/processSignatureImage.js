...
function withTimeout(promise, ms) {
    return Promise.race([
        promise,
        new Promise((_, reject) => setTimeout(() => reject(new Error(`Sharp processing timed out after ${ms}ms`)), ms)),
    ]);
}
async function runSharpPipeline(inputBuffer) {
    ...
    const adaptiveThreshold = Math.round(Math.min(200, Math.max(100, meanLuminance * 0.65)));
    ...
    for (let i = 0; i < width * height; i++) {
        ...
        if (lum > adaptiveThreshold) {
            ...
            const alpha = Math.round(255 * Math.min(1, (adaptiveThreshold - lum) / transitionZone));
    ...
    const finalBuffer = await sharp(Buffer.from(pixels.buffer, pixels.byteOffset, pixels.byteLength), { raw: { width, height, channels: ch } })
        .trim()
        .resize({
        width: 1200,
        height: 400,
...
export async function processSignatureImage(imageSource) {
    let inputBuffer;
    try {
        ...
        if (inputBuffer && inputBuffer.length > 0) {
            try {
                const fallback = await withTimeout(sharp(inputBuffer)
                    .rotate()
                    .resize({ width: 1200, height: 400, fit: 'inside' })
                    .png()
                    .toBuffer(), SHARP_TIMEOUT_MS);
